...
T-Orbit 是一个以**插件为核心**的桌面开发辅助平台，而非单一功能应用。宿主本身只负责插件加载、生命周期管理与导航渲染；所有业务能力均由可独立部署的插件提供。内置插件（设置、快捷键、插件监控）与目录扫描插件（如 EF Core 迁移工具）共享统一的设计系统与宿主服务，彼此相互隔离。
...
| **沉浸式设计系统** | Obsidian + Violet 调色板，全窗口沉浸布局，完整的主题、对话框、通用控件层（`Designer` 层），插件无需重复实现 |
...
宿主在启动时会扫描 `<AppDir>/plugins/` 目录，递归发现所有实现 `IPlugin` 的程序集并自动加载。外部插件的 `.csproj` 已配置将构建输出写入该目录。
...
- 深色 / 浅色主题切换
- 内置调色板（Tranbok Dark / Tranbok Light / Emerald Dark / Violet Dark）与自定义 JSON 主题
- 全局字体方案
- **插件变量管理**：集中配置各插件的环境变量键值对（支持加密字段）

### `torbit.monitor` — 插件监控

- 查看所有已注册插件的名称、版本、运行状态与标签
- 实时显示插件启停状态与异常信息
- 切换插件启用状态，调整导航排序
